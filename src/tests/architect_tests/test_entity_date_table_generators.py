...
def test_empty_output(db_engine):
    ...
    with db_engine.connect() as conn:
        (cohort_count,) = conn.execute(
            text(f"""select count(*) from {table_generator.entity_date_table_name}""")
        ).first()

    assert cohort_count == 0


def test_entity_date_table_generator_replace(db_engine):
    input_data = [
    ...
    utils.create_binary_outcome_events(db_engine, "events", input_data)
    table_generator = EntityDateTableGenerator(
        query="select entity_id from events where outcome_date < '{as_of_date}'::date",
        db_engine=db_engine,
    ...
    with db_engine.connect() as conn:
        results = list(conn.execute(text(f"""
                        select entity_id, as_of_date, active from {table_generator.entity_date_table_name}
                        order by entity_id, as_of_date
                    """)))
    assert results == expected_output
    utils.assert_index(db_engine, table_generator.entity_date_table_name, "entity_id")
    utils.assert_index(db_engine, table_generator.entity_date_table_name, "as_of_date")

    table_generator.generate_entity_date_table(as_of_dates)
    assert results == expected_output


def test_entity_date_table_generator_noreplace(db_engine):
    input_data = [
        (1, datetime(2016, 1, 1), True),
        (1, datetime(2016, 4, 1), False),
        (1, datetime(2016, 3, 1), True),
...
def test_entity_date_table_generator_from_labels(db_engine):
    labels_data = [
    ...
    labels_table_name = utils.create_labels(db_engine, labels_data)
    table_generator = EntityDateTableGenerator(
        query=None,
...
def test_subset_evens_cohort(db_engine):
    input_data = [
        (1, datetime(2016, 1, 1), True),
        (1, datetime(2016, 4, 1), False),
        (1, datetime(2016, 3, 1), True),
    ...
    with db_engine.begin() as conn:
        ...
        for row in input_data:
            conn.execute(
                text(f"insert into cohort values ({row[0]}, '{row[1]}', {row[2]})")
            )
    ...
    assert results == expected_output
    utils.assert_index(db_engine, subset_generator.entity_date_table_name, "entity_id")
    utils.assert_index(db_engine, subset_generator.entity_date_table_name, "as_of_date")


def test_subset_odds_cohort(db_engine):
    input_data = [
        (1, datetime(2016, 1, 1), True),
        (1, datetime(2016, 4, 1), False),
        (1, datetime(2016, 3, 1), True),
...
def test_subset_empty(db_engine):
    input_data = [
        (1, datetime(2016, 1, 1), True),
        (1, datetime(2016, 4, 1), False),
        (1, datetime(2016, 3, 1), True),
...
def test_subset_all_entities_in_cohort(db_engine):
    input_data = [
        (1, datetime(2016, 1, 1), True),
        (1, datetime(2016, 4, 1), False),
        (1, datetime(2016, 3, 1), True),
