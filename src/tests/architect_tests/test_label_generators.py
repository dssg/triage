...
def test_label_generation(db_engine):
    ...
    with db_engine.connect() as conn:
        result = conn.execute(
            text("select * from {} order by entity_id, as_of_date".format(LABELS_TABLE_NAME))
        )
        records = [tuple(row) for row in result]
...
def test_generate_all_labels_replace(db_engine):
    ...
    label_generator = LabelGenerator(db_engine=db_engine, query=LABEL_GENERATE_QUERY, replace=True)
...
def test_generate_all_labels_errors_on_duplicates(db_engine):
    ...
    label_generator = LabelGenerator(db_engine=db_engine, query=BAD_LABEL_GENERATE_QUERY, replace=True)
