...
def populate_source_data(db_engine):
    ...
    with db_engine.begin() as conn:
        ...
        conn.execute(text("""create table dog_complaints (
                entity_id int,
                as_of_date date,
                dog_sightings int
                )"""))
        ...
        for complaint in dog_complaints:
            conn.execute(
                text(
                    "insert into dog_complaints values (:entity_id, :as_of_date, :dog_sightings)"
                ),
                {
                    "entity_id": complaint[0],
                    "as_of_date": complaint[1],
        ...
        for event in events:
            conn.execute(
                ...
                {"entity_id": event[0], "outcome": event[1], "outcome_date": event[2]},
            )
...
def basic_integration_test(
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        label_generator = LabelGenerator(
            db_engine=db_engine, query=sample_config("query")["label_config"]["query"]
        )
        ...
        _, matrix_build_tasks = planner.generate_plans(split_definitions, feature_dicts)
        ...
        with db_engine.connect() as conn:
            matrices_records = list(
                conn.execute(text("""select matrix_uuid, num_observations, matrix_type
...
def test_integration_simple(db_engine):
    basic_integration_test(
        db_engine=db_engine,
        cohort_names=["mycohort"],
        feature_group_create_rules={
            "prefix": ["cat", "dog"]
        },  # Will be set by defaults.py
...
def test_integration_feature_grouping(db_engine):
    basic_integration_test(
        db_engine=db_engine,
        cohort_names=["mycohort"],
