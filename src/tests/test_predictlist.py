from triage.predictlist import (
    Retrainer, 
    predict_forward_with_existed_model
)
from triage.predictlist.utils import (
...
def test_predict_forward_with_existed_model_should_be_same_shape_as_cohort(finished_experiment):
    ...
    predict_forward_with_existed_model(
            ...
            model_id=model_id,
            as_of_date=as_of_date)
    # cohort coulb de -or not- configured in the experiment config file
    if 'cohort_config' not in finished_experiment.config:
        cohort_table_name = 'cohort_default' 
    else: 
        cohort_table_name = f"cohort_{finished_experiment.config['cohort_config']['name']}"

    with finished_experiment.db_engine.connect() as conn:
        num_records_matching_cohort = conn.execute( 
            text(f"""
                    select count(*)
                    from triage_production.predictions
                    join triage_production.{cohort_table_name} using (entity_id, as_of_date)
                """
            )
        ).first()[0]
    
        num_records = conn.execute(
            text('select count(*) from triage_production.predictions')
        ).first()[0]
...
def test_predict_forward_with_existed_model_matrix_record_is_populated(finished_experiment):
    ...
    with finished_experiment.db_engine.connect() as conn: 
        matrix_records = conn.execute(
            text("select * from triage_metadata.matrices where matrix_type = 'production'"),
        )
    
    assert len(list(matrix_records)) == 1
...
def test_experiment_config_from_model_id(finished_experiment):
    ...
    # if the cohort wasn't configured in the experiment config file, it will be set 
    # by the experiment run. Thus, the comparisson will fail. The original setup won't
    # have the cohort_table_name key.
    ...
    with finished_experiment.db_engine.connect() as conn:
        records = [
            ...
            for row in conn.execute(
                text(f"select model_hash from triage_metadata.models where model_comment = :model_comment"),
        ...
        matrix_records = conn.execute(
            text("select * from triage_metadata.matrices where matrix_uuid = :predict_matrix_uuid"), 
            {"predict_matrix_uuid": retrainer.predict_matrix_uuid},
        ).all()
