...
def test_imputation_output(feat_list, exp_imp_cols, feat_table, db_engine):
    with db_engine.connect() as conn:
        ...
        for state in states_table:
            conn.execute(
                ...
                {"entity_id": state[0], "as_of_date": state[1]}
            )
        ...
        for rec in feat_table:
            placeholders = ", ".join([f":p{i}" for i in range(len(rec))])
            params = {f"p{i}": v for i, v in enumerate(rec)}
            conn.execute(
                text(f"insert into prefix_aggregation values ({placeholders})"),
                params
            )
        conn.commit()
    ...
    for imp in available_imputations.keys():
        ...
        for coltype in ["aggregate", "categorical"]:
            ...
            # check the results
            df = pd.read_sql("SELECT * FROM prefix_aggregation_imputed", db_engine)
