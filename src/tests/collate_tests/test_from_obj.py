...
def test_materialized_from_obj_create():
    ...
    assert (
        materialized_query.create_materialized_table_sql
        == "create table myquery_from_obj as "
...
def test_materialized_from_obj_index():
    ...
    assert (
        materialized_query.index_materialized_table_sql
        == "create index on myquery_from_obj (knowledge_date)"
    )
...
def test_materialized_from_obj_drop():
    ...
    assert (
        materialized_query.drop_materialized_table_sql
        == "drop table if exists myquery_from_obj"
    )
...
def db_engine_with_events_table(db_engine):
    with db_engine.begin() as conn:
        ...
        for event in events_data:
            conn.execute(
                text(
                    "insert into events values (:entity_id, :event_date, cast(:outcome as bool))"
                ),
                {"entity_id": event[0], "event_date": event[1], "outcome": event[2]},
            )
        return db_engine
...
def test_materialized_from_obj_should_not_materialize_tbl_with_alias():
    from_obj = FromObj(
        from_obj="mytable1 as mt1", name="events", knowledge_date_column="date"
    )
    assert not from_obj.should_materialize()
...
def test_materialized_from_obj_should_not_materialize_join():
    from_obj = FromObj(
        from_obj="mytable1 join entities using(entity_id)",
        name="events",
...
def test_materialized_from_obj_should_materialize_subquery():
    from_obj = FromObj(
        from_obj="(select entity_id, date from mytable1 join entities using(entity_id)) joined_events",
        name="events",
        knowledge_date_column="date",
    )
...
def test_materialized_from_obj_maybe_materialize(db_engine_with_events_table):
    from_obj = FromObj(
        from_obj="events", name="myquery", knowledge_date_column="event_date"
    )
