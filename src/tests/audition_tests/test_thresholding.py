...
def setup_thresholder_data(engine):
    ensure_db(engine)
    init_engine(engine)
    ...
    with engine.connect() as conn:
        for dist_row in distance_rows:
            conn.execute(
                text("insert into dist_table values (:p1, :p2, :p3, :p4, :p5, :p6, :p7, :p8)"),
                {"p1": dist_row[0], "p2": dist_row[1], "p3": dist_row[2], "p4": dist_row[3],
                 "p5": dist_row[4], "p6": dist_row[5], "p7": dist_row[6], "p8": dist_row[7]}
            )
        conn.commit()
...
def test_thresholder_2014_close(db_engine):
    thresholder = setup_thresholder_data(db_engine)
