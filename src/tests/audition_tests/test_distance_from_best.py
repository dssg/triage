...
def test_DistanceFromBestTable(db_engine):
    ...
    distance_table = DistanceFromBestTable(
        db_engine=db_engine, models_table="models", distance_table="dist_table", agg_type="worst"
    )
    metrics = [
    ...
    query = """
        ...
        from dist_table where metric = :metric and parameter = :parameter and train_end_time = :train_end_time
    ...
    with db_engine.connect() as conn:
        prec_3y_ago = conn.execute(
            text(query),
            {"metric": "precision@", "parameter": "100_abs", "train_end_time": "2014-01-01"}
        )
        assert [tuple(row) for row in prec_3y_ago] == [
        ...
        recall_2y_ago = conn.execute(
            text(query),
            {"metric": "recall@", "parameter": "100_abs", "train_end_time": "2015-01-01"}
        )
        assert [tuple(row) for row in recall_2y_ago] == [
