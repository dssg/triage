...
def test_PreAudition(db_engine):
    ensure_db(db_engine)
    ...
    try:
        ...
        assert len(
            pre_aud.get_model_groups_from_label("label_1")["model_groups"]
        ) == sum([x["label_definition"] == "label_1" for x in model_configs])

        # Expect no baseline model groups
        assert (
            len(pre_aud.get_model_groups_from_label("label_1")["baseline_model_groups"])
            == 0
        )
        ...
        assert (
            len(
                pre_aud.get_model_groups_from_experiment(experiment_hash)[
                    "model_groups"
                ]
            )
            == 1
