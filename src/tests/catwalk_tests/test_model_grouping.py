...
def test_model_grouping_default_config(sample_metadata, db_engine):
    ensure_db(db_engine)
    model_grouper = ModelGrouper()
    # get the basic first model group with our default matrix
    assert (
        model_grouper.get_model_group_id(
            "module.Classifier", {"param1": "val1"}, sample_metadata, db_engine
        )
        == 1
    )
    ...
    assert (
        model_grouper.get_model_group_id(
            "module.Classifier", {"param1": "val1"}, metadata_new_end_time, db_engine
        )
        == 1
    )
    ...
    assert (
        model_grouper.get_model_group_id(
            "module.Classifier", {"param1": "val1"}, metadata_train_history, db_engine
        )
        == 2
    )
    ...
    assert (
        model_grouper.get_model_group_id(
            "module.OtherClassifier", {"param1": "val1"}, sample_metadata, db_engine
        )
        == 3
    )


def test_model_grouping_custom_config(sample_metadata, db_engine):
    ensure_db(db_engine)
    model_grouper = ModelGrouper(
    ...
    assert (
        model_grouper.get_model_group_id(
            "module.Classifier", {"param1": "val1"}, metadata_frequency, db_engine
        )
        == 2
    )
    ...
    assert (
        model_grouper.get_model_group_id(
            "module.Classifier", {"param1": "val1"}, metadata_features, db_engine
        )
        == 3
    )
