...
def test_baseline_exception_handling(default_model_trainer, db_engine):
    grid_config = {
    ...
    train_tasks = trainer.generate_train_tasks(
        grid_config, dict(), get_matrix_store(project_storage, db_engine)
    )
...
def test_custom_groups(grid_config, db_engine_with_results_schema, project_storage):
    ...
    model_ids = trainer.train_models(
        ...
        matrix_store=get_matrix_store(project_storage, db_engine_with_results_schema),
    )
    ...
    with db_engine_with_results_schema.connect() as conn:
        records = [
            ...
            for row in conn.execute(
                text("select distinct model_group_id from triage_metadata.models")
            )
        ]
...
def test_n_jobs_not_new_model(default_model_trainer):
    ...
    with db_engine.connect() as conn:
        for row in conn.execute(
            text("select hyperparameters from triage_metadata.model_groups")
        ):
...
def test_model_trainer(grid_config, default_model_trainer):
    ...
    with db_engine.connect() as conn:
        records = [
            row
            for row in conn.execute(
                text("select * from train_results.feature_importances")
            )
        ]
    ...
    with db_engine.connect() as conn:
        records = [
            row
            for row in conn.execute(text("select model_hash from triage_metadata.models"))
        ]
    ...
    with db_engine.connect() as conn:
        records = [
            row
            for row in conn.execute(
                text("select distinct random_seed from triage_metadata.models")
            )
        ]
    assert len(records) == 4
    ...
    with db_engine.connect() as conn:
        records = [
            row
            for row in conn.execute(text("select model_size from triage_metadata.models"))
        ]
    assert len(records) == 4
    for i in records:
        size = i[0]
        ...
        assert (
            len(
                [
                    row
                    for row in conn.execute(
                        text("select model_hash from triage_metadata.models")
                    )
                ]
            )
            == 4
    ...
    with db_engine.connect() as conn:
        max_batch_run_time = [
            row[0]
            for row in conn.execute(
                text("select max(batch_run_time) from triage_metadata.models")
            )
        ][0]
    experiment_hash = save_experiment_and_get_hash(
        config={'foo': 'bar'}, 
    ...
    with db_engine.connect() as conn:
        assert [
            row.model_id
            for row in conn.execute(
                text("select model_id from triage_metadata.models order by 1 asc")
            )
        ] == model_ids
...
def test_reuse_model_random_seeds(grid_config, default_model_trainer):
    ...
    print(set(new_model_ids))
    print(set(model_ids))
    # TODO the following assertion is not true, new model ids are different than previouos model ids which 
    # makes the len of (new model ids vs model ids) 0 and therefore != to the len of model ids
