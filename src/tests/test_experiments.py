    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.skip(
                reason="MultiCoreExperiment deadlocks with pytest-postgresql in CI"
            ),
        ),
    ],
)
...
def test_simple_experiment(experiment_class, db_engine):
    with (
        ...
        with db_engine.connect() as conn:
            ...
            num_models = len([row for row in conn.execute(text("""
            ...
            for set_type in ("train", "test"):
                num_predictions = len([row for row in conn.execute(text(f"""
            ...
            for set_type in ("train", "test"):
                num_evaluations = len([row for row in conn.execute(text(f"""
            ...
            num_models_with_experiment = len([row for row in conn.execute(text("""
            ...
            matrices = [row for row in conn.execute(text("""
...
def test_baselines_with_missing_features(experiment_class, db_engine):
    with (
        ...
        with db_engine.connect() as conn:
            ...
            num_predictions = len([row for row in conn.execute(text("""
            ...
            num_evaluations = len([row for row in conn.execute(text("""
...
def test_serializable_engine_check_sqlalchemy_fail(db_engine):
    ...
    with (
        ...
        # Get the URL from db_engine to create a vanilla sqlalchemy engine
...
def test_experiment_metadata(finished_experiment):
    ...
    try:
        ...
        assert (
            ...
            == experiment_row.time_splits
            * 2
            * experiment_row.feature_group_combinations
