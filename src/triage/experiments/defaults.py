...
def fill_timechop_config_missing(config, db_engine):
    ...
    if "label_timespans" in timechop_config.keys():
        if any(
            [
                k in timechop_config.keys()
                for k in ["training_label_timespans", "test_labels_timespans"]
            ]
        ):
            raise KeyError(
                "You can't always get what you want, but just sometimes, you get what you need: The config file has conflicting keys: the 'label_timespan' and 'training_label_timespans' and/or 'test_label_timespans'"
            )
        default_config["training_label_timespans"] = default_config[
            "test_label_timespans"
        ] = timechop_config["label_timespans"]
    ...
    if any(
        ...
        from_queries = [
            from_query.format(
                knowledge_date=agg["knowledge_date_column"], from_obj=agg["from_obj"]
            )
        ...
        query = "select to_char(min(min_date), 'YYYY-MM-DD'), to_char(max(max_date), 'YYYY-MM-DD') from ({unions}) as u".format(
            unions=unions
        )
        ...
        default_config["feature_start_time"] = default_config["label_start_time"] = (
            min_date
        )
    ...
    while prev_type is not None:
        ...
        for model_type in set(output.keys()).union(set(prev.keys())):
            ...
            for hyperparam in prev.get(model_type, {}).keys():
                curr_model[hyperparam] = sorted(
                    list(
                        set(
                            curr_model.get(hyperparam, [])
                            + prev[model_type][hyperparam]
                        )
                    ),
