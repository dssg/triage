...
def predict_forward_with_existed_model(
    db_engine, project_path, model_id, as_of_date, cohort_query=None
):
    ...
    if experiment_config.get("cohort_config") is None:
        ...
        logger.info(
            "Experiment config does not contain a cohort config. Using the label query"
        )
        experiment_config["cohort_config"] = cohort_config_from_label_config(
            experiment_config["label_config"]
        )

    cohort_table_name = (
        f"triage_production.cohort_{experiment_config['cohort_config']['name']}"
    )
    ...
    for aggregation in collate_aggregations:
        ...
        total_nonimpute_cols = (
            set(f for f in set(feature_names) if "_imp" not in f) - total_impute_cols
        )
    ...
    last_split_definition["first_as_of_time"] = str(
        last_split_definition["first_as_of_time"]
    )
    last_split_definition["last_as_of_time"] = str(
        last_split_definition["last_as_of_time"]
    )
    last_split_definition["matrix_info_end_time"] = str(
        last_split_definition["matrix_info_end_time"]
    )
    last_split_definition["as_of_times"] = [
        str(last_split_definition["as_of_times"][0])
    ]
    ...
    matrix_metadata["matrix_id"] = (
        str(as_of_date) + f"_model_id_{model_id}" + "_risklist"
    )
...
class Retrainer:
    ...
    def __init__(self, db_engine, project_path, model_group_id):
        ...
        self.model_group_info = get_model_group_info(
            self.db_engine, self.model_group_id
        )
        self.matrix_storage_engine = self.project_storage.matrix_storage_engine()
        self.triage_run_id, self.experiment_config = (
            experiment_config_from_model_group_id(self.db_engine, self.model_group_id)
        )
        ...
        self.training_label_timespan = self.experiment_config["temporal_config"][
        ...
        if self.experiment_config.get("cohort_config") is None:
            self.experiment_config["cohort_config"] = cohort_config_from_label_config(
                self.experiment_config["label_config"]
            )

        self.label_name = self.experiment_config["label_config"]["name"]

        # In case only the label config is created
        self.cohort_name = self.experiment_config.get("cohort_config", {}).get(
            "name", "default"
        )
    ...
    def get_temporal_config_for_retrain(self, prediction_date):
        ...
        temporal_config["model_update_frequency"] = "%syears" % (
            ...
            - dt_from_str(temporal_config["label_start_time"]).year
            + 10
        )
    ...
    def get_feature_dict_and_imputation_task(self, collate_aggregations, model_id):
        ...
        for aggregation in collate_aggregations:
            ...
            total_nonimpute_cols = (
                set(f for f in set(feature_names) if "_imp" not in f)
                - total_impute_cols
            )
    ...
    def retrain(self, prediction_date):
        ...
        as_of_date = datetime.strftime(
            chops_train_matrix["last_as_of_time"], "%Y-%m-%d"
        )
        retrain_definition = {
            ...
            "training_as_of_date_frequency": chops_train_matrix[
        ...
        logger.debug(
            f"Cohort query on retrainer: {self.experiment_config.get('cohort_config', {}).get('query')}"
        )
        ...
        # 4. Reconstruct feature disctionary from feature_names and generate imputation
        reconstructed_feature_dict, imputation_table_tasks = (
            self.get_feature_dict_and_imputation_task(
        ...
        last_split_train_matrix_uuid, last_split_matrix_metadata = (
            train_matrix_info_from_model_id(
        ...
        associate_models_with_retrain(
            self.retrain_hash, (retrain_model_hash,), self.db_engine
        )
    ...
    def predict(self, prediction_date):
        ...
        matrix_metadata["matrix_id"] = (
            str(prediction_date) + f"_model_id_{self.retrain_model_id}" + "_risklist"
        )
