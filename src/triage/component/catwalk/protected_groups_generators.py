...
class ProtectedGroupsGenerator:
    ...
    def generate_all_dates(self, as_of_dates, cohort_table_name, cohort_hash):
        ...
        if self.replace:
            ...
            logger.debug(
                f"Because replace flag is true, removed from {self.protected_groups_table_name} all rows from cohort {cohort_hash}"
            )
        ...
        for as_of_date in as_of_dates:
            if not self.replace:
                logger.spam(
                    f"Looking for existing protected_groups for as of date {as_of_date}"
                )
                with self.db_engine.connect() as conn:
                    result = conn.execute(
                        text(f"""select 1 from {self.protected_groups_table_name}
            ...
            logger.debug(f"Generating protected groups for as of date {as_of_date} ")
            self.generate(
        ...
        with self.db_engine.begin() as conn:
            ...
            nrows = conn.execute(
                text(f"select count(*) from {self.protected_groups_table_name}")
            ).scalar()
            ...
            if nrows == 0:
                ...
                logger.success(
                    f"Protected groups stored in the table "
    ...
    def generate(self, start_date, cohort_table_name, cohort_hash):
        ...
        logger.spam(
            f"Query to insert protected_df with start_date {start_date} and cohort_hash {cohort_hash}: {full_insert_query}"
        )
        with self.db_engine.begin() as conn:
            conn.execute(
    ...
    def as_dataframe(self, as_of_dates, cohort_hash):
        ...
        protected_df[self.attribute_columns] = protected_df[
