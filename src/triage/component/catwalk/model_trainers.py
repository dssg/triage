...
class ModelTrainer:
    ...
    def _train_and_store_model(
        ...
        if parameters.get('random_state', None): 
            logging.info("WARNING: User has defined a custom random seed, removing it from hyperparameters (still using it though!)")
    ...
    def generate_train_tasks(self, grid_config, misc_db_parameters, matrix_store=None):
        ...
        for class_path, parameters in self.flattened_grid_config(grid_config):
            ...
            if unique_parameters.get('random_state', None):
               logging.info(f"WARNING: User has explicitly defined a random seed for a particular model, overwriting the random seed generated by Triage from {random_seed} to {unique_parameters['random_state']}")
