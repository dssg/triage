...
def generate_risk_list(db_engine, matrix_storage_engine, model_storage_engine, model_id, as_of_date):
    ...
    with db_engine.begin() as conn:
        for aggregation in collate_aggregations:
            ...
            feature_group = aggregation.get_table_name(imputed=True).split('.')[1]
            feature_group = feature_group.replace('"', '')
            feature_names_in_group = [f for f in feature_names if re.match(f'\A{feature_prefix}', f)]
            reconstructed_feature_dictionary[feature_group] = feature_names_in_group

            feature_names_in_group = set(feature_names_in_group)
            features_imputed_in_train = set(f for f in feature_names_in_group if f + '_imp' in feature_names_in_group)

            results = conn.execute(aggregation.find_nulls())
            null_counts = results.first().items()

            features_imputed_in_production = set([col for (col, val) in null_counts if val > 0])

            total_impute_cols = features_imputed_in_production | features_imputed_in_train
            total_nonimpute_cols = set(f for f in feature_names_in_group if '_imp' not in f) - total_impute_cols
