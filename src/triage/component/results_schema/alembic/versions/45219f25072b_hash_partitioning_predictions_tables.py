...
def get_pg_major_version(op):
    conn = op.get_bind()
    pg_major_version = (
        conn.execute(text("show server_version")).scalar_one().split(".")[0]
    )
...
def upgrade():
    ...
    if pg_major_version >= 11:
        logger.info(
            f"PostgreSQL 11 or greater found (PostgreSQL {pg_major_version}): Using hash partitioning"
        )
        ...
        logger.info(
            f"No hash partitioning implemented because PostgreSQL 11 or greater not found (using: PostgreSQL {pg_major_version})"
        )
...
def downgrade():
    ...
    if pg_major_version >= 11:
        logger.info(
            f"PostgreSQL 11 or greater found  (PostgreSQL {pg_major_version}): Removing hash partitioning"
        )
        undo_hash_partitioning_filename = os.path.join(
            os.path.dirname(__file__),
            "../../sql/undo_predictions_hash_partitioning.sql",
        )
