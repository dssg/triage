...
class ExperimentReport:
    ...
    def cohorts(self, generate_plots=True):
        ...
        labels_table = pd.read_sql(
            get_labels_table_query, self.engine
        ).labels_table_name.iloc[0]
        # print(labels_table)
        cohort_query = f"""
            select 
                label_name, 
        ...
        if generate_plots:
            ...
            color = "darkblue"
            sns.lineplot(
                data=df, x="as_of_date", y="cohort_size", ax=ax1, label="", color=color
            )

            ax1.axhline(
                y=df.cohort_size.mean(),
                color=color,
                alpha=0.4,
                linestyle="--",
                label="mean cohort size",
            )
            ...
            sns.lineplot(data=df, x="as_of_date", y="baserate", ax=ax2, color=color)
    ...
    def model_groups(self):
        ...
        model_groups = pd.read_sql(
            mg_query_selected_exp.format("','".join(self.experiment_hashes)),
            self.engine,
        )
    ...
    def features(self):
        ...
        all_features = pd.DataFrame(all_features).sort_values(
            "feature_group", ignore_index=True
        )
    ...
    def feature_missingness(self):
        ...
        for table_name, columns in column_names.items():
            ...
            df = pd.read_sql(query, self.engine).set_index(
                ["as_of_date", "cohort_size"]
            )
    ...
    def model_performance(self, metric=None, parameter=None, generate_plot=True):
        ...
        models_per_train_end_time = df.groupby(
        ...
        if models_per_train_end_time[models_per_train_end_time > 1].empty:
            ...
            print(
                f"model groups with morel than one model id per train end time: \n{models_per_train_end_time[models_per_train_end_time > 1]}"
            )
    ...
    def model_performance_subsets(
        self, metric=None, parameter=None, generate_plot=True
    ):
        ...
        df["model_type_child"] = df.apply(
            lambda x: _format_model_name(x["model_type"], x["model_group_id"]), axis=1
        )
        df["model_type_short"] = df.apply(
            lambda x: x["model_type"].split(".")[-1], axis=1
        )
        ...
        if generate_plot:
            ...
            for grp, gdf in grpobj:
                ...
                subset_table_name = (
                    "subset_" + gdf["subset"].iloc[0] + "_" + gdf["subset_hash"].iloc[0]
                )
                q = f"""
                    select 
                ...
                axes[0].axhline(
                    y=subset_size.subset_size.mean(),
                    color=color,
                    alpha=0.4,
                    linestyle="--",
                )
                ...
                colors = {
                    x: sns.color_palette().as_hex()[i]
                    for i, x in enumerate(sorted(df.model_type_short.unique()))
                }
                legend_handles = [
                    Line2D([0], [0], label=k, color=v) for k, v in colors.items()
                ]
    ...
    def efficiency_and_equity(
        ...
        if groups is None:
            ...
            if rg.empty:
                logger.warning(
                    "No bias audit config or aequitas calculation was not completed! check the test_results.aequitas table. No plots generated"
                )
                return
            else:
                groups = dict()
        ...
        if model_group_ids is None:
            # logger.warning('No model groups specified. Usign all model group ids')
            model_group_ids = self.model_groups().model_group_id.tolist()

            if not model_group_ids:
                logger.warning(
                    "No model groups belong to the experiment! Returning None"
                )
                return None
        ...
        if groups is None:
            ...
            # logger.info('No groups are specified. Showing results for all attributes and their values')
            groups = dict()
            ...
            logger.debug(f"Plotting bias for following groups and values {rg}")
        ...
        metrics["model_label"] = metrics.apply(
            lambda x: _format_model_name(x["model_type"], x["model_group_id"]), axis=1
        )
        metrics["model_type_short"] = metrics.apply(
            lambda x: x["model_type"].split(".")[-1], axis=1
        )
        ...
        if generate_plot:
            # Metric means
            mean = (
                metrics.groupby(["model_label", "attribute_value", "model_type_short"])[
                    [f"{efficiency_metric}{parameter}", f"{equity_metric}"]
                ]
                .mean()
                .reset_index()
            ...
            colors = {
                x: sns.color_palette().as_hex()[i]
                for i, x in enumerate(sorted(metrics.model_type_short.unique()))
            }
            legend_handles = [
                Line2D([0], [0], label=k, marker="o", color=v)
                ...
                for attr in attrs:
                    ...
                    for i in range(len(x)):
                        axes[ax_cntr].errorbar(
                            ...
                            linewidth=1,
                            capsize=2,
                            color=colors[mean[msk]["model_type_short"].iloc[i]],
                            alpha=0.3,
                        )
                        axes[ax_cntr].scatter(
                            x[i],
                            y[i],
                            color=colors[mean[msk]["model_type_short"].iloc[i]],
                            label=labels[i],
                        ...
                        axes[ax_cntr].set(
                            title=f"{group} | {attr}",
                            xlabel="Efficiency Metric",
                            ylabel="Equity Metric",
                        )
                        axes[ax_cntr].axhline(
                            y=1, color="dimgray", linestyle="--", alpha=0.1
                        )
                        axes[ax_cntr].axhline(
                            y=1 + bias_metric_tolerance,
                            color="dimgray",
                            linestyle=":",
                            alpha=0.09,
                        )
            ...
            for group, attrs in groups.items():
                for attr in attrs:
                    ...
                    grouped = (
                        metrics[msk]
                        .groupby("train_end_time")[
                            ["baserate", "group_size", "total_entities"]
                        ]
                        .mean()
                        .reset_index()
                    )
    ...
    def generate_summary(self, metric=None, parameter=None, equity_metric=None):
        ...
        logger.notice(
            f"Default performance parameters are set to recall@1_pct and bias metric to tpr_disparity!"
        )
        logger.notice(
            "==> In case your experiment doesn't have those parameters Triage will use one of the available. <=="
        )
        stats = self.experiment_stats()

        if stats["implemented_fewer_splits"] == 1:
            logger.notice(
                f"Temporal config suggests {stats['timesplits_from_temporal_config']} temporal splits, but experiment implemented only {stats['validation_splits']} splits. Was this intentional?"
            )
        else:
            logger.notice(
                f'Experiment contained {stats["timesplits_from_temporal_config"]} temporal splits'
            )

        logger.notice(
            f"Experiment contained {stats['as_of_times']} distinct as_of_times"
        )

        cohorts = self.cohorts(generate_plots=False)
        logger.notice(
            f"On average, your cohorts contained around {round(cohorts.cohort_size.mean())} entities with a baserate of {round(cohorts.baserate.mean(), 3)}"
        )

        logger.notice(
            f"You built {stats['features']} features organized into {stats['feature_groups']} groups/blocks"
        )

        logger.notice(
            f"Your model grid specification contained {stats['grid_size']} model types with {stats['models_needed']} individual models"
        )
        ...
        if num_models < stats["models_needed"]:
            logger.notice(
                f"However, the experiment only built {num_models} models. You are missing {stats['models_needed'] - num_models} models"
            )
        ...
        if subset_performance is not None:
            ...
            for subset, gdf in grpobj:
                ...
                d["subset"] = subset
                d["best_perf"] = round(
                    gdf.groupby(["model_group_id", "model_type"])["metric_value"]
                    .mean()
                    .max(),
                    3,
                )
            ...
            if len(res) > 0:
                logger.notice(
                    f"You created {len(res)} subsets of your cohort -- {', '.join([x['subset'] for x in res])}"
                )
                for d in res:
                    logger.notice(
                        f"For subset '{d['subset'] }', Model Group {d['best_mod'][0]}, {d['best_mod'][1]} achieved the best average {metric}{parameter} of {d['best_perf']}"
                    )
        else:
        ...
        if equity_metrics is not None:
            ...
            for attr, gdf in grpobj:
                logger.notice(
                    f"Measuring biases across {attr} groups using {equity_metric} for the best performing model:"
                )
                d = gdf.groupby("attribute_value")[equity_metric].mean()
                logger.notice(
                    ", ".join(f"{k}: {round(v, 3)}" for k, v, in d.to_dict().items())
                )
        else:
            logger.notice(
                f"No bias audit results were found in the database for the experiment."
            )
    ...
    def precision_recall_curves(self, plot_size=(3, 3)):
        ...
        for _, gdf in grp_obj:
            ...
            for mod_id in model_ids:
                ...
                tmp.plot_precision_recall_curve(ax=axes[ax_idx])
                ax_idx += 1
    ...
    def get_best_hp_config_for_each_model_type(self):
        ...
        best_models = (
            pd.read_sql(q, self.engine)
            .set_index("model_group_id")
            .sort_values(by="mean_performance", ascending=False)
        )
        best_models["model_type"] = (
            best_models["model_type"].str.split(".").apply(lambda x: x[-1])
        )
    ...
    def feature_importance(self, plot_size=(2, 5), n_features=20):
        ...
        # n_groups x 1 grid
        fig, axes = plt.subplots(
            n_groups, 1, figsize=(plot_size[0], n_groups * plot_size[1])
        )
        ...
        for model_group_id, gdf in grp_obj:
            ...
            if len(feature_importances_group.feature.unique()) > 1:
                agg_df = (
                    feature_importances_group.groupby("feature")["feature_importance"]
                    .agg(["mean", "sem"])
                    .nlargest(n_features, "mean")
                    .reset_index()
                )
                ...
                with warnings.catch_warnings():
                    ...
                    axes[ax_idx].set_title(
                        f"{_format_model_name(tmp.model_type, model_group_id)}"
                    )
                ...
                logger.warning(f"Not a model with multiple features: {model_group_id} ")
