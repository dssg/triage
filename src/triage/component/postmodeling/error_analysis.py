...
def _get_error_analysis_configuration():
    ...
    filename_yaml = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "postmodeling_config.yaml"
    )
...
def _get_random_seed(model_id, db_conn):
    ...
    logger.debug(
        """obtained random seed associated to model_id: {model_id} for generating decision trees in error analysis"""
    )
...
def _fetch_matrices(model_id, project_path, db_conn):
    ...
    logger.info(f"extracting predictions from model id: {model_id}")
    predictions = _fetch_scores_labels(model_id, db_conn)
    matrix_uuid = predictions.matrix_uuid.unique()[0]
    logger.info(f"matrix uuid: {matrix_uuid}")
    ...
    logger.debug(
        f"predictions and labels joined for error analysis of model_id {model_id}"
    )
...
def _generate_error_labels(matrix, k):
    ...
    data_df["type_label"] = np.where(
        ~(data_df.label_value) & (data_df.prediction == "1"), "FP", "TP"
    )
    data_df["type_label"] = np.where(
        (data_df.label_value) & (data_df.prediction == "0"), "FN", data_df.type_label
    )
    data_df["type_label"] = np.where(
        ~(data_df.label_value) & (data_df.prediction == "0"), "TN", data_df.type_label
    )
    ...
    data_df["error_general_label"] = np.where(
        (data_df.type_label == "FP") | (data_df.type_label == "FN"), "1", "0"
    )
...
def _error_analysis_model(
    model_id, matrix, grid, k, random_seed, project_path, view_plots
):
    """
    ...
    error_analysis_types = [
        element for element in list(matrix.columns) if element.startswith("error")
    ]
    ...
    for error_type in error_analysis_types:
        ...
        if error_type == "error_negative_label":
            X = matrix[matrix.type_label.isin(["TN", "FN"])].drop(no_features, axis=1)
            y = matrix[matrix.type_label.isin(["TN", "FN"])].filter(
                [error_type], axis=1
            )
        elif error_type == "error_positive_label":
            X = matrix[matrix.type_label.isin(["TP", "FP"])].drop(no_features, axis=1)
            y = matrix[matrix.type_label.isin(["TP", "FP"])].filter(
                [error_type], axis=1
            )
        else:
        ...
        for config in ParameterGrid(grid):
            ...
            top_n = [
                i
                for i, element in enumerate(importances_idx)
                if feature_importances_[element] == 0
            ][0]
            ...
            config_params["tree_text"] = export_text(
                error_model, feature_names=list(feature_), show_weights=True
            )

            _plot_feature_importance(
                config_params["feature_importance"],
            ...
            logger.debug(
                f"error analysis generated for model_id {model_id}, with k {k}"
            )
...
def generate_error_analysis(model_id, db_conn, project_path):
    ...
    for k in k_set:
        ...
        error_analysis_result = _error_analysis_model(
            model_id, new_matrix, grid, k, random_seed, project_path, view_plots
        )
...
def _plot_feature_importance(
    importances, features, error_type, project_path, k, max_depth, model_id, view_plots
):
    """
    ...
    sns.barplot(data=df_viz_, x="importance", y="feature", color="grey")
    ...
    if view_plots:
        ...
        # save plot
        feature_importance_filename = "_".join(
            ["feature_importance", error_type, str(k), str(max_depth)]
        )
        ...
        project_storage = ProjectStorage(project_path)
        storage = project_storage.get_store(
            ["error_analysis/feature_importance/" + str(model_id)],
...
def _plot_feature_importance_local(importances, features, error_label):
    ...
    a = sns.barplot(data=df_viz_, x="importance", y="feature", color="grey")
    a.set_title("Top feature importance in " + error_label + " analysis.")
    plt.show()


def _generate_tree_graphviz(
    error_model,
    error_type,
    feature_names,
    k,
    ...
    if view_plots:
        ...
        project_storage = ProjectStorage(project_path)
        storage = project_storage.get_store(
            ["error_analysis/tree/" + str(model_id)], dot_path
        )
        storage.write(img_data.read())
...
def output_all_analysis(error_analysis_results):
    ...
    for analysis in error_analysis_results:
        for element in analysis:
            ...
            _plot_feature_importance_local(
                element["feature_importance"], element["feature_names"], error_label
            )
...
def output_specific_configuration(
    error_analysis_resutls, k=100, max_depth=10, error_type="error_negative_label"
):
    """
...
def output_specific_error_analysis(
    error_analysis_results, error_type="error_negative_label"
):
    """
...
if __name__ == "__main__":
    model_id = 1417
    db_conn = psycopg.connect(service="acdhs_housing")
    # its given by the repor generator
    project_path = (
        "s3://dsapp-social-services-migrated/acdhs_housing/triage_experiments/"
    )
