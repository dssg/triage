...
class SingleModelAnalyzer:
    ...
    @cachedproperty
    def metadata(self):
        return next(self.engine.execute(text(f"""
    ...
    def get_predictions(
        ...
        if preds.empty:
            logger.warning(
                f"No predictions were found in {predictions_table} for model_id {self.model_id}. Returning empty dataframe!"
            )
            # raise RuntimeError(
    ...
    def get_feature_importances(self, n_top_features=20):
        ...
        logger.debug(
            f"Fetching feature importance from db for model id: {self.model_id}"
        )
    ...
    def get_feature_group_importances(self):
        ...
        feature_groups = [
            i["prefix"] for i in pd.read_sql(q, self.engine)["feature_groups"].iloc[0]
        ]
        feature_groups
        case_part = ""
        for fg in feature_groups:
            case_part = case_part + "\nWHEN feature like '{fg}%%' THEN '{fg}'".format(
                fg=fg
            )

        # get feature group importances
        feature_group_importance = pd.read_sql(
            f"""
    ...
    def crosstabs_pos_vs_neg(
        ...
        positive_support_pct = lambda pos, neg: round(
            (pos > 0).sum(axis=0).astype(float) / len(pos), 3
        )
        negative_support_pct = lambda pos, neg: round(
            (neg > 0).sum(axis=0).astype(float) / len(neg), 3
        )
        ...
        if matrix_uuid is None:
            ...
            logger.debug(f"Matrix uuid set to: {matrix_uuid}")

        predictions = self.get_predictions(
            matrix_uuid=matrix_uuid, predictions_table=predictions_table
        )

        if predictions.empty:
            logger.error(
                f"No predictions found for {self.model_id} and {matrix_uuid}. Exiting!"
            )
        ...
        if table_exists(f"test_results.{table_name}", self.engine):
            ...
            logger.debug(
                f"Checking whether crosstabs already exist for the model {self.model_id} and {matrix_uuid}"
            )
            ...
            if not df.empty:
                logger.warning(
                    f"Crosstabs aleady exist for model {self.model_id} and matrix_uuid={matrix_uuid}"
                )

                if replace:
                    logger.warning("Deleting the existing crosstabs!")
                    ...
                    logger.info(
                        f"Replace set to False. Not calculating crosstabs for model {self.model_id} and matrix_uuid='{matrix_uuid}';"
                    )
                    if return_df:
                        return df
                    else:
        ...
        for threshold_name, threshold in thresholds.items():
            logger.info(f"Crosstabs using threshold: {threshold_name} <= {threshold}")
        ...
        if push_to_db:
            logger.info(
                f"Pushing the results to the database, {len(crosstabs_df)} rows"
            )
            ...
            # Use psycopg3 COPY for efficient bulk insert
            ...
            try:
                ...
                with cursor.copy(
                    f"COPY test_results.{table_name} ({columns}) FROM STDIN WITH CSV"
                ) as copy:
    ...
    def display_crosstabs_pos_vs_neg(
        ...
        pivot_table = pd.pivot(ct, index="feature", columns="metric", values="value")
        ...
        if show_plot:
            if ax is None:
                fig, ax = plt.subplots(
                    figsize=(4, 1 + (display_n_features * 2) / 5), dpi=100
                )
    ...
    def error_analysis(self, project_path):
        ...
        error_analysis_results = generate_error_analysis(
            model_id, connection, project_path=project_path
        )
        # TODO do we want to
    ...
    def plot_score_distribution(self, ax, nbins=10, top_k=None, matrix_uuid=None):
        ...
        ax.hist(predictions.score, bins=nbins, alpha=0.5, color="blue")
        ax.axvline(predictions.score.mean(), color="black", linestyle="dashed")
    ...
    def plot_score_label_distribution(
        self,
        ax,
        nbins=10,
        top_k=None,
        ...
        # keep only top_k if specified
        if top_k:
            df_predictions = df_predictions.loc[
                df_predictions["rank_abs_no_ties"] <= top_k
            ]
        df__0 = df_predictions[df_predictions.label_value == 0]
        df__1 = df_predictions[df_predictions.label_value == 1]

        ax.hist(
            df__0.score, bins=nbins, alpha=0.5, color="skyblue", label=label_names[0]
        )
        ax.hist(
            list(df__1.score),
            bins=nbins,
        ...
        ax.axvline(df__0.score.mean(), color="skyblue", linestyle="dashed")
        ax.axvline(df__1.score.mean(), color="orange", linestyle="dashed")
        ax.legend(
            bbox_to_anchor=(0.0, 1.005, 1.0, 0.102), loc=8, ncol=2, borderaxespad=0.0
        )
        ax.set_ylabel("Frequency")
        ax.set_xlabel("Score")
    ...
    def plot_bias_threshold_curve(
        self, attribute_name, attribute_value, bias_metric, ax
    ):
        bias_df = self.get_aequitas()
        # limit to specific attribute+value
        bias_df = bias_df.loc[
            (bias_df["attribute_name"] == attribute_name)
            & (bias_df["attribute_value"] == attribute_value)
        ]

        bias_df["perc_points"] = [
            x.split("_")[0] for x in bias_df["parameter"].tolist()
        ]
    ...
    def plot_precision_threshold_curve(self, ax, matrix_uuid=None):
        ...
        eval_df["perc_points"] = [
            x.split("_")[0] for x in eval_df["parameter"].tolist()
        ]
    ...
    def plot_precision_recall_curve(
        ...
        # handle ties
        pred_df_score, pred_df_label, pred_df_index = sort_predictions_and_labels(
            pred_df["score"],
            pred_df["label_value"],
            pred_df.index,
            tiebreaker="random",
        ...
        pred_df = pd.DataFrame(
            list(zip(pred_df_score, pred_df_label)), columns=["score", "label_value"]
        )
        ...
        if pred_df.empty or subset_hash is not None:
            ...
            eval_df = self.get_evaluations(
                matrix_uuid=matrix_uuid, subset_hash=subset_hash
            )

            if eval_df.empty:
                logger.error("No evaluations were found! Returning empty axes!")
                return ax
            ...
            logger.debug(
                f"Found saved predictions for model id {self.model_id}.(group: {self.model_group_id})"
            )
            k_values = np.arange(
                0, list_size_upper_bound_pct + pct_step_size, pct_step_size
            )
            ...
            for k in k_values:
                ...
                precision = (
                    pred_pos.label_value.sum() / num_above_thresh
                    if num_above_thresh > 0
                    else 0
                )
                recall = (
                    pred_pos.label_value.sum() / num_positives
                    if num_above_thresh > 0
                    else 0
                )
            ...
            with sns.axes_style("darkgrid"):
                if not only_recall:
                    sns.lineplot(
                        x=k_values * 100, y=precisions, ax=ax, label="Precision@k"
                    )
    ...
    def plot_calibration_curve(
        self, ax, nbins=20, min_score=0, max_score=1, matrix_uuid=None
    ):
        """
        ...
        cal_x, cal_y = calibration_curve(
            scores["label_value"], scores["score"], n_bins=nbins
        )
        sns.lineplot(x=cal_x, y=cal_y, marker="o", ax=ax)
        ...
        sns.lineplot(
            data=pd.DataFrame(
                list(zip(probabilities, probabilities)), columns=["x", "y"]
            ),
            x="x",
            y="y",
            color="gray",
...
class MultiModelAnalyzer:
    ...
    def __init__(
        ...
        experiment_hashes,
        project_path=None,
        train_end_times=None,
        use_all_model_groups=False,
    ) -> None:
        ...
        if (
            use_all_model_groups
        ):  # shortcut to listing out all model groups for an experiment
    ...
    def display_model_groups(self):
            ...
            for train_end_time in self.models[mg]:
                ...
                data_dict.append(
                    [
                        mg,
                        train_end_time,
                        model_analyzer.model_id,
                        model_analyzer.model_type,
                        model_analyzer.hyperparameters,
                    ]
                )
        ...
        to_print = (
            all_models.groupby("model_group_id")
            .nth(1)[["model_type", "hyperparameters"]]
            .reset_index()
    ...
    def print_model_summary(self):
        ...
        for i, model in (
            ...
            print(
                f"{model['model_group_id']} - {model['model_type']} with ({model['hyperparameters']}) "
            )
        ...
        for experiment_hash in self.experiment_hashes:
            ...
            for m in models:
                if m["model_group_id"] in d:
                    d[m["model_group_id"]][m["train_end_time"]] = SingleModelAnalyzer(
                        m["model_id"], self.engine
                    )
                else:
                    d[m["model_group_id"]] = {
                        m["train_end_time"]: SingleModelAnalyzer(
                            m["model_id"], self.engine
                        )
                    }
    ...
    def _make_plot_grid(
        ...
        **kw,
        ...
        fig, axes = self._get_subplots(
            ...
            subplot_len=subplot_len,
            sharey=sharey,
            sharex=sharex,
        )

        logger.debug(f"Plot grid dimensions: {len(axes)}, {len(axes[0])}")
    ...
    def plot_recall_curves_overlaid(
        self,
        n_splits=None,
        **kw,
    ):
        ...
        for i, aod in enumerate(as_of_dates):
            for model_group in sorted(self.model_groups):
                ...
                mod.plot_precision_recall_curve(
                    only_recall=True, ax=axes[i], title_string=aod
                )
    ...
    def plot_precision_threshold(self):
        ...
        if len(self.models) <= 1:
            print(
                "Not available when there is only one model group (look at plot_prk_curves instead)"
            )
            return
        ...
        for _, mg in enumerate(self.models):
            for j, train_end_time in enumerate(self.models[mg]):
                mode_analyzer = self.models[mg][train_end_time]
                mode_analyzer.plot_precision_threshold_curve(ax=axes[j])
    ...
    def plot_feature_importance(self, n_top_features=20):
        ...
        self._make_plot_grid(
            plot_type="plot_feature_importance",
            subplot_width=7,
            n_top_features=n_top_features,
        )
    ...
    def plot_feature_group_importance(self, n_top_groups=20):
        ...
        self._make_plot_grid(
            plot_type="plot_feature_group_importance",
            subplot_width=7,
            n_top_groups=n_top_groups,
        )

    def create_scatter_disparity_performance(
        self,
        metric,
        parameter,
        aeq_parameter,
        attr_col,
        attribute_values,
        performance_col="stochastic_value",
        bias_metric="tpr",
        tiebreaker="worst",
        flip_disparity=False,
        mitigated_tags=[],
        mitigated_bdfs=[],
        mitigated_performances=[],
        ylim=None,
    ):
        """
        ...
        for k, attribute_value in enumerate(attribute_values):
            for i, mg in enumerate(self.models):
                for j, train_end_time in enumerate(self.models[mg]):
                    ...
                    evals_df_list[train_end_time].append(
                        mode_analyzer.get_evaluations(metrics={metric: [parameter]})
                    )  # TODO only allow one eval metric here?
                    aequitas_df_list[train_end_time].append(
                        mode_analyzer.get_aequitas()
                    )
            ...
            for j, train_end_time in enumerate(self.models[self.model_groups[0]]):
                ...
                aequitas_df = aequitas_df.loc[
                    (aequitas_df["parameter"] == aeq_parameter)
                    * (aequitas_df["tie_breaker"] == tiebreaker)
                ]
                disparity_df = aequitas_df.loc[
                    (aequitas_df["attribute_name"] == attr_col)
                    & (aequitas_df["attribute_value"] == attribute_value)
                ].copy()
                ...
                if flip_disparity:
                    disparity_df[disparity_metric] = disparity_df.apply(
                        lambda x: 1 / x[disparity_metric], axis=1
                    )
                scatter = pd.merge(
                    evals_df,
                    disparity_df,
                ...
                scatter.sort_values(
                    "stochastic_value", ascending=False, inplace=True, ignore_index=True
                )
                scatter_final = pd.DataFrame()

                ax = axes[k, j]
                ax.scatter(x="stochastic_value", y=disparity_metric, data=scatter)
                if not scatter_final.empty:
                    ax.scatter(
                        x="stochastic_value", y=disparity_metric, data=scatter_final
                    )
        ...
        fig.suptitle(
            f"{flip_placeholder} {disparity_metric} vs. {metric}{parameter} for {attr_col}"
        )

    def calculate_crosstabs_pos_vs_neg(
        self, project_path, thresholds, table_name="crosstabs", **kwargs
    ):
        """Generate crosstabs for the predicted positives (top-k) vs the rest
        ...
        for i, mg in enumerate(self.models):
            for j, train_end_time in enumerate(self.models[mg]):
                ...
                try:
                    ...
                    logger.error(
                        "Please run calculate_crosstabs_pos_vs_neg function to calculate crosstabs first for all models!"
                    )
                    raise e

                print(
                    f"\nModel Group: {mg}, Validation date: {train_end_time}".center(
                        30, " "
                    )
                )
                display(df)
    ...
    def _pairwise_feature_importance_comparison_single_split(
        self, train_end_time, n_top_features, model_group_ids=None, plot=True
    ):
        ...
        if model_group_ids is not None:
            ...
            for mg in model_group_ids:
                feature_lists[mg] = self.models[mg][
                    train_end_time
                ].get_feature_importances(n_top_features=n_top_features)
                if feature_lists[mg].empty:
                    logger.warning(
                        "No feature importance values were found for model group {mg}. Excluding from comparison"
                    )
        ...
        # By default all feature importance values are considered
            ...
            for mg, m in self.models.items():
                feature_lists[mg] = m[train_end_time].get_feature_importances(
                    n_top_features=n_top_features
                )
                if feature_lists[mg].empty:
                    logger.warning(
        ...
        logger.info(f"Performing {len(pairs)} comparisons")
        ...
        for m in metrics:
            results[m] = pd.DataFrame(
                index=sorted(model_group_ids), columns=sorted(model_group_ids)
            )
        ...
        for model_group_pair in pairs:
            ...
            logger.info(f"Comparing {model_group_pair[0]} and {model_group_pair[1]}")
            ...
            if (len(f1) == 0 or len(f2)) == 0:
                logger.error("No feature importance available for the models!")
                continue

            inter = f1.intersection(f2)
            un = f1.union(f2)
            results["jaccard"].loc[model_group_pair[1], model_group_pair[0]] = len(
                inter
            ) / len(un)

            # If the list sizes are not equal, using the smallest list size to calculate simple overlap
            results["overlap"].loc[model_group_pair[1], model_group_pair[0]] = len(
                inter
            ) / min(len(f1), len(f2))
            ...
            results["rank_corr"].loc[model_group_pair[0], model_group_pair[1]] = (
                spearmanr(df1.feature.iloc[:], df2.feature.iloc[:])[0]
            )
    ...
    def _pairwise_list_comparison_single_fold(
        self, threshold_type, threshold, train_end_time, matrix_uuid=None, plot=True
    ):
        ...
        for mg, m in self.models.items():
            lists[mg] = m[train_end_time].get_top_k(
                threshold_type, threshold, matrix_uuid
            )
        ...
        for m in metrics:
            results[m] = pd.DataFrame(
                index=sorted(self.model_groups), columns=sorted(self.model_groups)
            )
            results[m].values[[np.arange(results[m].shape[0])] * 2] = 1
        ...
        for model_group_pair in pairs:
            ...
            if (len(entities_1) == 0 or len(entities_2)) == 0:
                logger.error("No prediction saved for the models!")
            ...
            results["overlap"].loc[model_group_pair[1], model_group_pair[0]] = len(
                inter
            ) / min(len(entities_1), len(entities_2))
            ...
            if len(df1) == len(df2):
                results["rank_corr"].loc[model_group_pair[0], model_group_pair[1]] = (
                    spearmanr(df1.entity_id.iloc[:], df2.entity_id.iloc[:])[0]
                )
            else:
                logger.warning(
                    f"Not calculating rank correlation. List sizes are not equal ({len(df1)}, {len(df2)})"
                )
    ...
    def pairwise_feature_importance_comparison(
        self, n_top_features, model_groups=None, train_end_times=None, plot=True
    ):
        """
        Compare the top-k lists for the given train_end_times for all model groups considered (pairwise)
