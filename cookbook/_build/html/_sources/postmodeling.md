...
```{image} images/project_life_cycle.png
...
+ [Crosstabs](#crosstabs-target) 
+ [Lists similarities](#lists-similarities-target)
+ [Score distribution](#score-distribution-target)
+ [Label distribution](#label-distribution-target)
+ [List characteristics](#list-characteristics-target) 
+ [Outcomes on label window](#outcomes-on-label-window-target)
+ [Outcomes after label window](#outcomes-after-label-window-target)
+ [Error analysis FPs] 
+ [Error analysis FNs] 
+ [Basic extra recipes](#basic-extra-recipes-target)
...
+ A `model id`
+ Predictions generated and stored in the database. In case you don't have them you can follow [this recipe](#generating-predictions-store-in-db-target).
+ A connection to the DB. In case you don't have one you can follow [this recipe](#creating-a-database-engine-target)
...
1. **[Jaccard similarity](#jaccard-similarity-target):** This is telling you how similar are the lists based on the set created by the union of two lists. [recipe](#jaccard-similarity-target)
2. **[Overlap](#overlap-target):** How much intersection is between a pair of top k lists. This is telling you how many entities are present in both lists. You can also think about this as a way to see if the lists are highlighting different entities. This is very useful when you're trying to answer if one model could replace another one &ndash;like a current process implemented by the partner&ndash;.
3. **[Rank correlation](#rank-correlation-target):** How similar is the ranking between lists. This is telling you if the order in which the entities in one top *k* list is roughly the same &ndash;increasing or decresing monotonically (1, -1)&ndash; or if the order doesn't have any correlation at all(value of 0). 

We'll go through the recipes of each metric: 

(jaccard-similarity-target)=
#### Jaccard similiarity
...
(list-characteristics-target)=
### List characteristics

(outcomes-on-label-window-target)=
### Outcomes on label window 

(outcomes-after-label-window-target)=
### Outcomes after label window

(basic-extra-recipes-target)=
### Basic extra recipes

+ [Creating a database engine](#creating-a-database-engine-target)
+ [Generating predictions and storing them in DB after the experiment has run in Triage](#generating-predictions-store-in-db-target)

(creating-a-database-engine-target)=
...
(generating-predictions-store-in-db-target)=
